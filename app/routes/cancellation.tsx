...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const submissionData = { 
    ...
    email, 
    userId: 1,
    bookingDetails: "Mock cancellation details" 
  };
  ...
  try {
    ...
    return json({ success: true, message: "Cancellation request submitted successfully!" });
  } catch (error) {
    console.error("Error submitting cancellation:", error);
    return json({ success: false, message: "Failed to submit cancellation request" }, { status: 500 });
  }
}
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow flex items-center justify-center bg-background p-4">
        <div className="w-full max-w-md space-y-6">
          <h1 className="text-3xl font-bold text-center">Cancel Session</h1>
